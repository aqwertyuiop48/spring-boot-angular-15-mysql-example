...
```
apt update && apt-get install -y netcat && apt-get install -y mysql-client && apt-get install -y lsof && apt install openjdk-21-jdk -y && export JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64 && export PATH=$JAVA_HOME/bin:$PATH && apt install -y nodejs npm && git config --global --add safe.directory /workspaces/spring-boot-angular-15-mysql-example && git config --global user.email "aqwertyuiop48@hotmail.com" && git config --global user.name "aqwertyuiop48" && git remote set-url origin https://aqwertyuiop48@github.com/aqwertyuiop48/spring-boot-angular-15-mysql-example.git && git add . && git commit -m "C" && git push origin master
```
